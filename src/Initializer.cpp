                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    // mvMatches12记录匹配上的特征点对 i值为参考帧,记录值为currentframe
    mvMatches12.clear();
    mvMatches12.reserve(mvKeys2.size());
    // mvbMatched1记录每个特征点是否有匹配的特征点，
    // 这个变量后面没有用到，后面只关心匹配上的特征点
    mvbMatched1.resize(mvKeys1.size());  //记录参考帧中哪个参与了匹配.

    // 步骤1：组织特征点对,由vMatches12搞一个pair
    ...
    int bsizex = 128;
    int bsizey = 120;
    int nGridrows = CurrentFrame.mnMaxY / bsizey;
    int nGridcols = CurrentFrame.mnMaxX / bsizex;
    vector<int> mGrid[nGridcols][nGridrows];

    for (int i = 0; i < mvMatches12.size(); i++)  //利用匹配较好的点.
    {
        if (mvMatches12[i] > 0) {
            int posX = CurrentFrame.mvKeysUn[i].pt.x / bsizex;
            int posY = CurrentFrame.mvKeysUn[i].pt.y / bsizey;
    ...
    // 步骤5：从H矩阵或F矩阵中恢复R,t
    //vbMatchesInliersH or vbMatchesInliersF 代表可成功三角化的点.
    return ReconstructF(vbMatchesInliersF,F,mK,R21,t21,vP3D,vbTriangulated,1.0,50);  //返回的是一个bool值;
...
{   
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(chiSquare2>th) //所谓卡方检验 意味这该数据有多少概率符合该模型;
            bIn = false;
        else
            score += th - chiSquare2;
                      ...
                      cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    cv::Mat invK = K.inv();
    cv::Mat A = invK*H21*K;  //所以H21不是真正的H模型,而是G=K*H*inv(K).
