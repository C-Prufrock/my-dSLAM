...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // 步骤2：添加顶点：待优化当前帧的Tcw
    g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw)); //此处将位姿转换为四元数；
    vSE3->setId(0);
    vSE3->setFixed(false);
    optimizer.addVertex(vSE3);
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(pFrame->mvuRight[i]<0)
            {
                ...
                obs << kpUn.pt.x, kpUn.pt.y;  //构成边的观测量;
                ...
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);//设置观测矩阵 并将当前帧的尺度信息添加到单位矩阵中;

                g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
                e->setRobustKernel(rk);
                rk->setDelta(deltaMono);  //设置鲁邦核的参数,参数与单目的内点阈值相关;
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw));//设置估计量 初值，//5t由于李群不具备可加性（对加法不闭合）;
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            const float chi2 = e->chi2(); //若误差大于阈值，则视为外点，剔除ｇ２ｏ优化图；

            if(chi2>chi2Mono[it])
            {                
                pFrame->mvbOutlier[idx]=true;
                e->setLevel(1);                 // 设置为outlier
                nBad++;  //有多少外点,计算出后,减去得到最终位姿优化后的满足运动模型的内点;
            }
            else
            {
