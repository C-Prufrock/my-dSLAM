        ...
        if(KFit->first == Fit->first) //步骤1：分别取出属于同一node的ORB特征点(只有属于同一node，才有可能是匹配点)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                for(size_t iF=0; iF<vIndicesF.size(); iF++)
                {
                    ...
                    if(vpMapPointMatches[realIdxF])// 表明这个点已经被匹配过了，不再匹配，加快速度;想像本次循环为第二循环即可;
                        continue;
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    {
        MapPoint* pMP = vpMapPoints[i]; //遍历当前帧的地图点.
        ...
        cv::Mat p3Dw = pMP->GetWorldPos();  //获得当前地图点的位置;
        cv::Mat p3Dc = Rcw*p3Dw + tcw;     //地图点在相连关键帧的位置坐标系.
