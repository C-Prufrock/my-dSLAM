        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    ...
                    bOK = TrackReferenceKeyFrame(); //返回了bool值; 如果上一帧未被成功跟踪,则利用相关参考帧进行跟踪;
                }
                else
                {
                    // 根据恒速模型设定当前帧的初始位姿
                    ...
                    bOK = TrackWithMotionModel();            //利用上一帧图形进行跟踪;
                    if(!bOK)
                        // TrackReferenceKeyFrame是跟踪参考帧，不能根据固定运动速度模型预测当前帧的位姿态，通过bow加速匹配（SearchByBow）
                        // 最后通过优化得到优化后的位姿
                        bOK = TrackReferenceKeyFrame();      //如果利用恒速运动模型跟踪失败,则采用相关关键帧进行跟踪;
                }
            }
            else
            {
                // BOW搜索，PnP求解位姿
                bOK = Relocalization();                    //否则进行重定位;
            }
        }
        else //局部地图不工作,只进行跟踪;
        {
    ...
    else
    {
        ...
        //vbTriangulated保存了可被成功三角化的点.
        {
            ...
            for(size_t i=0, iend=mvIniMatches.size(); i<iend;i++)
            {
                ...
                {
                    mvIniMatches[i]=-1;  //匹配点已经是足可以三角化的点.
                    nmatches--;
                }
            }
            cout<< "nmathces after poseCaculate: " <<nmatches<<endl;
            // Set Frame Poses
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        MapPoint* pMP = new MapPoint(worldPos,pKFcur,mpMap); //将三角化出的3D点搞成Mappoint;
    ...
    cout<<"Tracked Map Points: "<<pKFcur->TrackedMapPoints(1)<<endl;
    ...
    mpLocalMapper->InsertKeyFrame(pKFini); //插入第一关键帧
    mpLocalMapper->InsertKeyFrame(pKFcur);  //插入第二关键帧

    mCurrentFrame.SetPose(pKFcur->GetPose()); //获得位姿
    mnLastKeyFrameId=mCurrentFrame.mnId;     //将当前帧id赋给上一个关键帧;
    ...
    mpMap->SetReferenceMapPoints(mvpLocalMapPoints); //设置为参考地图点;
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    if(nmatches<15) //如果当前帧与参考的关键帧之间匹配数目小于 15则返回flase;
        return false; 
...
bool Tracking::TrackLocalMap()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!mCurrentFrame.mvbOutlier[i])            //确定不是模型外点;
            {
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
                if(!mbOnlyTracking)
                {
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        for(vector<MapPoint*>::const_iterator itMP=vpMPs.begin(), itEndMP=vpMPs.end(); itMP!=itEndMP; itMP++)
        {
            ...
            if(!pMP->isBad())
            {
                ...
                pMP->mnTrackReferenceForFrame=mCurrentFrame.mnId;//如果不用上述才做,则pMP->mnTrackReferenceForFrame的值仍是上一帧的id;
            }
        }
    }
}
...
void Tracking::UpdateLocalKeyFrames()
{
        ...
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!pMP->isBad())
            {
                ...
                for(map<KeyFrame*,size_t>::const_iterator it=observations.begin(), itend=observations.end(); it!=itend; it++)
                    keyframeCounter[it->first]++;//可获得共视关键帧的个数,知道第一个地图点的共视关键帧之后即不再考虑该关键帧是否对下个地图点共视;
            }
            else
            {
    ...
    if(keyframeCounter.empty())
        return;//若对于该地图点没有共视关键帧,说明只是由于上一帧图像和当前帧图像三角化出来的,则不做处理;
...
bool Tracking::Relocalization()
{
    ...
    vpPnPsolvers.resize(nKFs); //建立备选帧个数的PnPsolvers;

    vector<vector<MapPoint*> > vvpMapPointMatches;
    vvpMapPointMatches.resize(nKFs);

    vector<bool> vbDiscarded; //判断要丢弃哪些备选关键帧;
