...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
    ...
    // 步骤1：找出和当前帧具有公共单词的所有关键帧
    {
        ...
        for(DBoW2::BowVector::const_iterator vit=F->mBowVec.begin(), vend=F->mBowVec.end(); vit != vend; vit++)
        {
            ...
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++) //因为关键帧之间可能共享很多特征点;
            {
                KeyFrame* pKFi=*lit;
                if(pKFi->mnRelocQuery!=F->mnId)// pKFi还没有标记为pKF的候选帧
                {
                    pKFi->mnRelocWords=0;      //找到的第一个关键帧共视帧; 因为后面有++,因此这里置0,因为之前与其他关键帧计算过共视关系,这里一定不为0;
