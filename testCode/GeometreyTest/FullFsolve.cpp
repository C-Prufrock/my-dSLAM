//
// Created by true on 19-4-1.
//

//所谓Ｆullsolve，即在获取相机的初始位姿的时候，不利用Ｒansac的方式，而是利用全部的特征点信息进行位姿求解；
//类似一种smooth手段．
...
int main()
{
    ...
    cout<<"coutnumber : "<<countnumber<<endl;
    ...
    //迭代变量
    cv::Mat F21;
    vector<cv::Point2f> vPn1i(N);
    vector<cv::Point2f> vPn2i(N);
    cv::Mat F21i;
    ...
    for(int i=0;i<vbCurrentInliers.size();i++)
    {
        if(vbCurrentInliers[i]){
            cv::Point2f p1, p2;
    ...
    if(ComputeRTForF(R,t,F21i,keypoints1,keypoints2,vbMatchesInliers,K))
    {
        cout<<"R"<<R<<endl;
        cout<<"t"<<t<<endl;
    ...
    cout<<"numberGoodPoint : "<<numberGoodPoint<<endl;
    ...
    cout<<"CurrentModelInliers: "<<CurrentModelInliers.size()<<endl;
    ...
    CheckFundamental(F21,keypoints1,keypoints2,vbMatchesInliers);
    ...
    for(int i=0;i<keypoints1.size();i++)
    {
        if(p3dC1List[i].at<float>(0)!=0.0 && p3dC1List[i].at<float>(1)!=0.0)
        {
            In3Dnumber++;
        }

    }
    cout<<"In3Dnumber : "<<In3Dnumber<<endl;
