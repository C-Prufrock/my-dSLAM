   ...
   int bestidx = 0;
    ...
    for (int it = 0; it < nMaxIterations; it++) {
        ...
        currentScore = CheckDominatFundamental(F21i, mvIniMatches, mCurrentFrame1.mvKeysUn, GridHas[iGrid],
                                               mCurrentFrame2.mvKeysUn, 3.871, sigma, vbCurrentInliers,vbCurrentInliersInAllFeatures,
                                               it);
    ...
    cout<<"得到的最优F: "<<endl<<F21<<"score :"<<score<<endl;
    ...
    if(ComputeRTForF(R, t, F21Final, mCurrentFrame1.mvKeysUn, mCurrentFrame2.mvKeysUn, vbMatchesInliersInAllFeatures, K,
                      mvIniMatches))
    {
    ...
    {
        float diff=MOdelDiffBetweenGrid(RList[0],RList[i],TList[0],TList[i]);
