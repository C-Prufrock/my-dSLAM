...
int main() {
    ...
    //建立特征提取器
    ORB_SLAM2::ORBextractor *mpORBextractor = new ORBextractor(3.5 * ORBExtractorParameter->nFeatures,
    ...
    */
    ...
    if(inliersnumber>8){
        ...
        //cout << "H21Final with all inliers :" << endl << H21Final << endl;
        //存储格子中的H模型；
        //cout<<"H21 : "<<H21Final<<endl;
        H21list.push_back(H21Final);
    }
    else
    {
        cout<<" 模型内点数目为" << inliersnumber<<" 求解H模型不正确 "<<endl;
        continue;
    }
    ...
    cout << "final,inliersnumber2: " << inliersnumber2 << endl;
    ...
    cout<<" 共得到了" <<" "<<H21list.size()<<" 个模型 "<<endl;
    ...
    //验证模型内点之间的相互符合程度;
    //记录格子中的模型内点;
    cout<<"H21list.size() : "<<H21list.size()<<endl;
    cv::Mat coupleMatrix = cv::Mat::zeros(H21list.size(), H21list.size(), CV_32F);
    //cout<<"H21listinv : "<<H21listinv<<endl;
    for(int i=0;i<H21list.size();i++){
        cv::Mat H21listinv=H21list[i].inv();
        for(int j=0;j<H21list.size();j++){
            //cout<<"check number :"<<checkModelCoupling(H21list[i],H21listinv,vbMatchInliersList1[j],vbMatchInliersList2[j])<<endl;
            coupleMatrix.at<float>(i,j)=checkModelCouplingH(H21list[i],H21listinv,vbMatchInliersList1[j],vbMatchInliersList2[j]);
        }
    }
    cout<<"coupleMatrix  :"<<"  "<<endl<<coupleMatrix<<endl;
